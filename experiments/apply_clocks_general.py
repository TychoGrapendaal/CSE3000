...
adata = adata[adata.obs['cell_type'] == cell_type].to_memory()
print(f"{cell_type} shape: {adata.shape}")
...
def predict(data, cell_type, dataset_name):
    ...
    impute_data = pd.read_csv("trained_clocks/Impute_avg_"+cell_type+".csv")
    models_celltype = pd.read_csv("trained_clocks/"+cell_type+"_models5.csv")
